...
**MEK scope by security level:** The **Memory Encryption Key (MEK)** is an optional runtime key intended for
presence-bound “dead man’s switch” designs. MEK is **not required and normally disabled** for security levels **0–5**.
MEK becomes **required only at Level 6** when Network Presence Binding (NPB) / distributed secret sharing is enabled.
Levels 0–5 MUST NOT rely on MEK for the confidentiality of persisted data; they rely on CMK/DBK/DEK/TDE and standard
...
Each encrypted page header MUST store:
- `key_version` (16-bit or larger),
- `page_enc_epoch` (32-bit monotonic counter scoped to `(page_id, key_version)`).

The IV MUST be constructed as exactly 96 bits:

- `IV = page_id_64 || key_version_16 || page_enc_epoch_32`

Where:
- `page_id_64` is a stable, unique 64-bit identifier for the page within the database (e.g., hash of tablespace UUID + page number),
- `key_version_16` is the active DEK/TDE key version used for this page,
- `page_enc_epoch_32` increments **before** each write of the encrypted page under the same key_version and is persisted with the page.
...
For IPC AEAD, IV uniqueness is enforced by per-channel sequence numbers:
- `IV = channel_nonce_prefix_32 || seq_64`

The `channel_nonce_prefix_32` MUST be derived from the channel’s HKDF context (e.g., `HKDF(..., info="ipc-nonce-prefix")`),
and `seq_64` MUST be strictly monotonic per direction.
...
The LEK encrypts write-after log (WAL) entries (optional):
