...
Wrapper types: JSONB uses `scratchbird::Jsonb`, RANGE uses `scratchbird::Range<T>`, GEOMETRY uses `scratchbird::Geometry`. Use these wrappers when binding parameters or reading results.
