...
**Normative notes (applies to this entire matrix):**
- **System catalog (`sys.*`) access:** In **strict mode**, direct SQL access to `sys.*` is **denied** (including for superusers). Introspection is provided via **engine-owned virtual views** and `SHOW/DESCRIBE` commands (Option A).
- **MEK:** The Memory Encryption Key is **required only at Level 6** when NPB / distributed secret sharing is enabled. Levels 0–5 MUST NOT depend on MEK for persisted data confidentiality.
