 ...
 */
#ifndef SCRATCHROBIN_ISSUE_TRACKER_GITHUB_H
#define SCRATCHROBIN_ISSUE_TRACKER_GITHUB_H
...
// ============================================================================
class GitHubAdapter : public IssueTrackerAdapter {
public:
    static constexpr const char* PROVIDER_NAME = "github";
    
    ~GitHubAdapter() override = default;
...
private:
    ...
    HttpResponse HttpPatch(const std::string& path, const std::string& body);
    
    IssueReference ParseIssue(const std::string& json_response);
    IssueStatus ParseGitHubState(const std::string& state);
    std::string StatusToGitHub(IssueStatus status);
};

} // namespace scratchrobin

#endif // SCRATCHROBIN_ISSUE_TRACKER_GITHUB_H
