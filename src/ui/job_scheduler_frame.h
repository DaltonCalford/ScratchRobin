#ifndef SCRATCHROBIN_JOB_SCHEDULER_FRAME_H
#define SCRATCHROBIN_JOB_SCHEDULER_FRAME_H

#include <string>
#include <vector>

#include <wx/frame.h>
#include <wx/grid.h>

#include "core/connection_manager.h"
#include "core/query_types.h"

class wxChoice;
class wxGrid;
class wxStaticText;
class wxTextCtrl;
class wxButton;
class wxNotebook;

namespace scratchrobin {

class ResultGridTable;
class WindowManager;
struct AppConfig;

class JobSchedulerFrame : public wxFrame {
public:
    JobSchedulerFrame(WindowManager* windowManager,
                      ConnectionManager* connectionManager,
                      const std::vector<ConnectionProfile>* connections,
                      const AppConfig* appConfig);

private:
    void BuildMenu();
    void BuildLayout();
    void PopulateConnections();
    const ConnectionProfile* GetSelectedProfile() const;
    bool EnsureConnected(const ConnectionProfile& profile);
    bool IsNativeProfile(const ConnectionProfile& profile) const;
    void UpdateControls();
    void UpdateStatus(const wxString& status);
    void SetMessage(const std::string& message);

    void RefreshJobs();
    void RefreshJobDetails(const std::string& job_name);
    void RefreshJobRuns(const std::string& job_name);
    void RefreshJobGrants(const std::string& job_name);

    void RunCommand(const std::string& sql, const std::string& success_message);

    std::string GetSelectedJobName() const;
    std::string GetSelectedRunId() const;
    int FindColumnIndex(const QueryResult& result, const std::vector<std::string>& names) const;
    std::string ExtractValue(const QueryResult& result, int row, const std::vector<std::string>& names) const;
    std::string FormatJobDetails(const QueryResult& result) const;

    void OnConnect(wxCommandEvent& event);
    void OnDisconnect(wxCommandEvent& event);
    void OnRefresh(wxCommandEvent& event);
    void OnJobSelected(wxGridEvent& event);
    void OnRunSelected(wxGridEvent& event);
    void OnCreate(wxCommandEvent& event);
    void OnEdit(wxCommandEvent& event);
    void OnDrop(wxCommandEvent& event);
    void OnExecute(wxCommandEvent& event);
    void OnCancelRun(wxCommandEvent& event);
    void OnGrant(wxCommandEvent& event);
    void OnRevoke(wxCommandEvent& event);
    void OnNewSqlEditor(wxCommandEvent& event);
    void OnNewDiagram(wxCommandEvent& event);
    void OnOpenMonitoring(wxCommandEvent& event);
    void OnOpenUsersRoles(wxCommandEvent& event);
    void OnOpenDomainManager(wxCommandEvent& event);
    void OnOpenSchemaManager(wxCommandEvent& event);
    void OnOpenTableDesigner(wxCommandEvent& event);
    void OnOpenIndexDesigner(wxCommandEvent& event);
    void OnClose(wxCloseEvent& event);

    WindowManager* window_manager_ = nullptr;
    ConnectionManager* connection_manager_ = nullptr;
    const std::vector<ConnectionProfile>* connections_ = nullptr;
    const AppConfig* app_config_ = nullptr;

    wxChoice* connection_choice_ = nullptr;
    wxButton* connect_button_ = nullptr;
    wxButton* disconnect_button_ = nullptr;
    wxButton* refresh_button_ = nullptr;
    wxButton* create_button_ = nullptr;
    wxButton* edit_button_ = nullptr;
    wxButton* drop_button_ = nullptr;
    wxButton* run_button_ = nullptr;
    wxButton* cancel_button_ = nullptr;
    wxButton* grant_button_ = nullptr;
    wxButton* revoke_button_ = nullptr;
    wxTextCtrl* grant_principal_ctrl_ = nullptr;
    wxStaticText* status_text_ = nullptr;
    wxTextCtrl* message_text_ = nullptr;
    wxTextCtrl* details_text_ = nullptr;

    wxGrid* jobs_grid_ = nullptr;
    wxGrid* runs_grid_ = nullptr;
    wxGrid* grants_grid_ = nullptr;
    ResultGridTable* jobs_table_ = nullptr;
    ResultGridTable* runs_table_ = nullptr;
    ResultGridTable* grants_table_ = nullptr;

    int active_profile_index_ = -1;
    int pending_queries_ = 0;
    QueryResult jobs_result_;
    QueryResult runs_result_;
    QueryResult grants_result_;
    QueryResult job_details_result_;
    std::string selected_job_;

    wxDECLARE_EVENT_TABLE();
};

} // namespace scratchrobin

#endif // SCRATCHROBIN_JOB_SCHEDULER_FRAME_H
