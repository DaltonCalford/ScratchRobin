 ...
 */
#ifndef SCRATCHROBIN_STORAGE_MANAGER_FRAME_H
#define SCRATCHROBIN_STORAGE_MANAGER_FRAME_H
...
class StorageManagerFrame : public wxFrame {
public:
    StorageManagerFrame(WindowManager* windowManager,
...
private:
    ...
    void RefreshTablespaces();
    void RefreshTablespaceObjects(const std::string& tablespace_name);
    void RefreshTablespaceStats(const std::string& tablespace_name);
    void RunCommand(const std::string& sql, const std::string& success_message);

    std::string GetSelectedTablespaceName() const;
    ...
    std::string FormatStats(const QueryResult& result) const;

    void OnConnect(wxCommandEvent& event);
    void OnDisconnect(wxCommandEvent& event);
    void OnRefresh(wxCommandEvent& event);
    void OnTablespaceSelected(wxGridEvent& event);
    ...
    wxButton* relocate_button_ = nullptr;
    wxStaticText* status_text_ = nullptr;
    wxTextCtrl* message_text_ = nullptr;

    // Main tablespace grid
    wxGrid* tablespaces_grid_ = nullptr;
    ResultGridTable* tablespaces_table_ = nullptr;

    // Notebook for tabs
    wxNotebook* notebook_ = nullptr;

    // Objects tab
    wxGrid* objects_grid_ = nullptr;
    ResultGridTable* objects_table_ = nullptr;
    wxStaticText* objects_label_ = nullptr;
    ...
    QueryResult tablespaces_result_;
    QueryResult objects_result_;
    QueryResult stats_result_;
    std::string selected_tablespace_;
...
#endif // SCRATCHROBIN_STORAGE_MANAGER_FRAME_H
