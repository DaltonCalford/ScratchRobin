 ...
 */
#ifndef SCRATCHROBIN_PROCEDURE_MANAGER_FRAME_H
#define SCRATCHROBIN_PROCEDURE_MANAGER_FRAME_H
...
public:
    ProcedureManagerFrame(WindowManager* windowManager,
...
private:
    ...
    void RefreshRoutineDefinition(const std::string& routine_name);
    void RefreshParameters(const std::string& routine_name);
    void RefreshDependencies(const std::string& routine_name);
    void RunCommand(const std::string& sql, const std::string& success_message);

    std::string GetSelectedRoutineName() const;
    std::string GetSelectedRoutineType() const;
    ...
    void OnRoutineSelected(wxGridEvent& event);
    void OnFilterChanged(wxCommandEvent& event);
    void OnCreateProcedure(wxCommandEvent& event);
    void OnCreateFunction(wxCommandEvent& event);
    ...
    wxButton* create_procedure_button_ = nullptr;
    wxButton* create_function_button_ = nullptr;
    ...
    wxGrid* routines_grid_ = nullptr;
    ResultGridTable* routines_table_ = nullptr;

    wxNotebook* notebook_ = nullptr;
    wxTextCtrl* definition_text_ = nullptr;
    wxGrid* parameters_grid_ = nullptr;
    ResultGridTable* parameters_table_ = nullptr;
    wxTextCtrl* dependencies_text_ = nullptr;

    int active_profile_index_ = -1;
    int pending_queries_ = 0;
    QueryResult routines_result_;
    QueryResult parameters_result_;
    std::string selected_routine_;
    std::string selected_routine_type_;
...
#endif // SCRATCHROBIN_PROCEDURE_MANAGER_FRAME_H
