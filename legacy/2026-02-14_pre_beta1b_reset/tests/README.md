...
This directory contains the test suite for ScratchRobin, organized into smoke, unit, and integration test binaries.
...
├── run_tests.sh                # Test runner (labels + timeouts)
...
cmake ..
cmake --build . --target scratchrobin_tests_smoke scratchrobin_tests_unit scratchrobin_tests_integration
```

### Run Default Suite (Smoke + Unit)
...
```bash
./scratchrobin_tests_unit --gtest_filter="MetadataModelTest.*"
```

### Run with Verbose Output

```bash
./scratchrobin_tests_unit --gtest_verbose
```
