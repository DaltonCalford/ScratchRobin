...
void DiagramPage::BuildLayout() {
    ...
    auto* refresh_data_views_button = new wxButton(propsPanel, wxID_ANY, "Refresh All Data Views...");
    propsSizer->Add(refresh_data_views_button, 0, wxEXPAND | wxLEFT | wxRIGHT | wxBOTTOM, 8);

    auto* refresh_single_data_view_button =
        new wxButton(propsPanel, wxID_ANY, "Refresh Data View...");
    propsSizer->Add(refresh_single_data_view_button, 0, wxEXPAND | wxLEFT | wxRIGHT | wxBOTTOM, 8);

    data_view_status_label_ = new wxStaticText(propsPanel, wxID_ANY, "Data Views: 0 (stale: 0)");
    data_view_status_label_->SetForegroundColour(wxColour(180, 180, 180));
    propsSizer->Add(data_view_status_label_, 0, wxLEFT | wxRIGHT | wxBOTTOM, 8);
    ...
    refresh_data_views_button->Bind(wxEVT_BUTTON, &DiagramPage::OnRefreshDataViews, this);
    refresh_single_data_view_button->Bind(wxEVT_BUTTON, &DiagramPage::OnRefreshSingleDataView, this);
    canvas_->Bind(wxEVT_CHAR_HOOK, &DiagramPage::OnCanvasKey, this);
    canvas_->Bind(wxEVT_BUTTON, &DiagramPage::OnRefreshSingleDataView, this);
    canvas_->Bind(EVT_DATA_VIEW_UPDATED, &DiagramPage::OnDataViewUpdated, this);
    canvas_->Bind(EVT_DATA_VIEW_OPEN, &DiagramPage::OnOpenDataViewEditor, this);
    UpdateDataViewStatus();
    SyncDataViewsToCanvas();
}
...
void DiagramPage::OnCanvasKey(wxKeyEvent& event) {
    ...
    if (!event.ControlDown() && canvas_->HandleDataViewKey(key)) {
        return;
    }
...
void DiagramPage::OnOpenTrace(wxCommandEvent&) {
    ...
    if (dialog.ShowModal() == wxID_OK) {
        ...
        for (auto* win : wxTopLevelWindows) {
            auto* frame = dynamic_cast<DiagramFrame*>(win);
            if (!frame) {
                continue;
            }
            if (frame->FocusNodeInOpenDiagrams(ref.ToStdString(), DiagramType::Erd)) {
                frame->Raise();
                frame->Show(true);
                opened = true;
                break;
        ...
        if (!opened) {
            for (auto* win : wxTopLevelWindows) {
                ...
                if (frame->OpenDiagramFromTrace(ref.ToStdString())) {
                    frame->Raise();
                    frame->Show(true);
                    opened = true;
                    break;
        ...
        if (!opened) {
            wxMessageBox("Trace target not found in catalog or diagrams: " + ref,
                         "Trace", wxOK | wxICON_WARNING, this);
        }
    }
}

void DiagramPage::OnRefreshDataViews(wxCommandEvent&) {
    auto project = ProjectManager::Instance().GetCurrentProject();
    if (!project) {
        wxMessageBox("No active project for data view refresh.", "Data Views",
    ...
    std::string reason;
    size_t refreshed = 0;
    for (const auto& view : project->data_views) {
        if (project->RefreshDataViewWithGovernance(view.id, "", context, &reason)) {
            refreshed++;
        }
    }
    if (refreshed == 0 && !project->data_views.empty() && !reason.empty()) {
        wxMessageBox("Refresh blocked: " + reason, "Data Views",
                     wxOK | wxICON_WARNING, this);
        return;
    }
    wxMessageBox(wxString::Format("Data Views refreshed: %zu", refreshed),
                 "Data Views", wxOK | wxICON_INFORMATION, this);
    UpdateDataViewStatus();
    SyncDataViewsToCanvas();
}

void DiagramPage::OnRefreshSingleDataView(wxCommandEvent& event) {
    wxString direct_id = event.GetString();
    if (!direct_id.empty()) {
        auto project = ProjectManager::Instance().GetCurrentProject();
        if (project) {
            Project::GovernanceContext context;
            ...
            context.role = "designer";
            std::string reason;
            UUID target = UUID::FromString(direct_id.ToStdString());
            if (!project->RefreshDataViewWithGovernance(target, "", context, &reason)) {
                wxMessageBox("Refresh blocked: " + reason, "Data Views",
                             wxOK | wxICON_WARNING, this);
            }
            UpdateDataViewStatus();
    ...
    if (project->data_views.empty()) {
        wxMessageBox("No Data Views found in project.", "Data Views",
                     wxOK | wxICON_INFORMATION, this);
        return;
    }
    ...
    std::vector<UUID> ids;
    for (const auto& view : project->data_views) {
        std::string label = view.id.ToString();
        JsonParser parser(view.json_payload);
        JsonValue root;
        std::string error;
        bool stale = false;
        if (parser.Parse(&root, &error)) {
            if (const JsonValue* name = FindMember(root, "name")) {
                if (name->type == JsonValue::Type::String && !name->string_value.empty()) {
                    label = name->string_value + " (" + label + ")";
                }
            }
            if (const JsonValue* stale_val = FindMember(root, "stale")) {
                if (stale_val->type == JsonValue::Type::Bool && stale_val->bool_value) {
                    stale = true;
                }
            }
        }
    ...
    wxSingleChoiceDialog dialog(this, "Select a Data View to refresh", "Data Views", choices);
    if (dialog.ShowModal() != wxID_OK) {
        return;
    }
    int selection = dialog.GetSelection();
    if (selection < 0 || static_cast<size_t>(selection) >= ids.size()) {
        return;
    }
    ...
    context.role = "designer";
    std::string reason;
    if (!project->RefreshDataViewWithGovernance(ids[selection], "", context, &reason)) {
        wxMessageBox("Refresh blocked: " + reason, "Data Views",
                     wxOK | wxICON_WARNING, this);
        return;
    }
    wxMessageBox("Data View refreshed.", "Data Views",
                 wxOK | wxICON_INFORMATION, this);
    UpdateDataViewStatus();
    SyncDataViewsToCanvas();
}

void DiagramPage::OnDataViewUpdated(wxCommandEvent& event) {
    std::string payload = event.GetString().ToStdString();
    if (payload.empty()) return;
    auto project = ProjectManager::Instance().GetCurrentProject();
    if (!project) return;

    size_t p1 = payload.find('|');
    size_t p2 = payload.find('|', p1 == std::string::npos ? p1 : p1 + 1);
    size_t p3 = payload.find('|', p2 == std::string::npos ? p2 : p2 + 1);
    if (p1 == std::string::npos || p2 == std::string::npos || p3 == std::string::npos) {
        return;
    }
    std::string id_str = payload.substr(0, p1);
    int scroll = std::stoi(payload.substr(p1 + 1, p2 - p1 - 1));
    int sel_row = std::stoi(payload.substr(p2 + 1, p3 - p2 - 1));
    int sel_col = std::stoi(payload.substr(p3 + 1));

    UUID id = UUID::FromString(id_str);
    for (auto& view : project->data_views) {
        if (view.id == id) {
            std::string updated = view.json_payload;
            UpdateJsonObjectNumberField(updated, "scroll_row", scroll, &updated);
            UpdateJsonObjectNumberField(updated, "selected_row", sel_row, &updated);
            UpdateJsonObjectNumberField(updated, "selected_col", sel_col, &updated);
...
void DiagramPage::OnOpenDataViewEditor(wxCommandEvent& event) {
    std::string id_str = event.GetString().ToStdString();
    ...
    for (auto& view : project->data_views) {
        if (view.id == id) {
            DataViewEditorDialog dialog(this, view.json_payload);
            if (dialog.ShowModal() == wxID_OK) {
                std::string payload = dialog.json_payload();
                ...
                if (parser.Parse(&root, &parse_error)) {
                    ...
                    if (query_value && query_value->type == JsonValue::Type::String &&
                        ...
                        if (refs.parsed && !refs.identifiers.empty()) {
                            std::string updated;
                            if (UpdateJsonObjectStringArrayField(payload, "query_refs",
                                                                 refs.identifiers, &updated)) {
                                payload = updated;
                            }
                        }
                    }
                }
                view.json_payload = payload;
                project->UpsertDataView(view);
                UpdateDataViewStatus();
                SyncDataViewsToCanvas();
            }
    ...
    wxMessageBox("Data View not found for editor.", "Data Views",
                 wxOK | wxICON_WARNING, this);
}

void DiagramPage::SyncDataViewsToCanvas() {
    if (!canvas_) return;
    auto project = ProjectManager::Instance().GetCurrentProject();
    if (!project) {
        canvas_->SetDataViews({});
        return;
    }

    std::vector<DiagramCanvas::DataViewPanel> panels;
    std::string diagram_id = doc_.diagram_id;
    for (const auto& view : project->data_views) {
        if (!diagram_id.empty() && view.diagram_id.ToString() != diagram_id) {
            continue;
        }
        JsonParser parser(view.json_payload);
        JsonValue root;
        ...
        DiagramCanvas::DataViewPanel panel;
        panel.id = view.id.ToString();
        panel.name = "Data View";
        double x = 40.0;
        double y = 40.0;
        double w = 280.0;
        double h = 160.0;
        if (const JsonValue* name_val = FindMember(root, "name")) {
            if (name_val->type == JsonValue::Type::String && !name_val->string_value.empty()) {
                panel.name = name_val->string_value;
            }
        }
        if (const JsonValue* pos_val = FindMember(root, "position")) {
            if (pos_val->type == JsonValue::Type::Object) {
                if (const JsonValue* x_val = FindMember(*pos_val, "x")) {
                    if (x_val->type == JsonValue::Type::Number) x = x_val->number_value;
                }
                if (const JsonValue* y_val = FindMember(*pos_val, "y")) {
                    if (y_val->type == JsonValue::Type::Number) y = y_val->number_value;
                }
            }
        }
        if (const JsonValue* size_val = FindMember(root, "size")) {
            if (size_val->type == JsonValue::Type::Object) {
                if (const JsonValue* w_val = FindMember(*size_val, "width")) {
                    if (w_val->type == JsonValue::Type::Number) w = w_val->number_value;
                }
                if (const JsonValue* h_val = FindMember(*size_val, "height")) {
                    if (h_val->type == JsonValue::Type::Number) h = h_val->number_value;
                }
            }
        }
        if (const JsonValue* cols_val = FindMember(root, "columns")) {
            if (cols_val->type == JsonValue::Type::Array) {
                for (const auto& col : cols_val->array_value) {
                    if (col.type == JsonValue::Type::Object) {
                        if (const JsonValue* name_val = FindMember(col, "name")) {
                            if (name_val->type == JsonValue::Type::String) {
                                panel.columns.push_back(name_val->string_value);
                            }
                        }
                        if (const JsonValue* type_val = FindMember(col, "type")) {
                            if (type_val->type == JsonValue::Type::String) {
                                panel.column_types.push_back(type_val->string_value);
                            } else {
                                panel.column_types.push_back("");
                            }
                        } else {
                            panel.column_types.push_back("");
                        }
        ...
        if (const JsonValue* rows_val = FindMember(root, "rows")) {
            if (rows_val->type == JsonValue::Type::Array) {
                for (const auto& row : rows_val->array_value) {
                    if (row.type != JsonValue::Type::Array) continue;
                    std::vector<std::string> values;
                    for (const auto& cell : row.array_value) {
                        if (cell.type == JsonValue::Type::String) {
                            values.push_back(cell.string_value);
                        } else if (cell.type == JsonValue::Type::Number) {
                            values.push_back(std::to_string(cell.number_value));
                        } else if (cell.type == JsonValue::Type::Bool) {
                            values.push_back(cell.bool_value ? "true" : "false");
                        } else {
                            values.push_back("");
                        }
                    }
                    panel.rows.push_back(values);
                }
            }
        }
        if (const JsonValue* stale_val = FindMember(root, "stale")) {
            if (stale_val->type == JsonValue::Type::Bool) {
                panel.stale = stale_val->bool_value;
            }
        }
        if (const JsonValue* scroll_val = FindMember(root, "scroll_row")) {
            if (scroll_val->type == JsonValue::Type::Number) {
                panel.scroll_row = static_cast<int>(scroll_val->number_value);
            }
        }
        if (const JsonValue* sel_row_val = FindMember(root, "selected_row")) {
            if (sel_row_val->type == JsonValue::Type::Number) {
                panel.selected_row = static_cast<int>(sel_row_val->number_value);
            }
        }
        if (const JsonValue* sel_col_val = FindMember(root, "selected_col")) {
            if (sel_col_val->type == JsonValue::Type::Number) {
                panel.selected_col = static_cast<int>(sel_col_val->number_value);
            }
        }
        panel.rect = wxRect2DDouble(x, y, w, h);
        panels.push_back(panel);
    }
    canvas_->SetDataViews(panels);
}

void DiagramPage::UpdateDataViewStatus() {
    if (!data_view_status_label_) return;
    auto project = ProjectManager::Instance().GetCurrentProject();
    if (!project) {
        data_view_status_label_->SetLabel("Data Views: 0 (stale: 0)");
        return;
    }
    size_t total = project->data_views.size();
    size_t stale = 0;
    for (const auto& view : project->data_views) {
        JsonParser parser(view.json_payload);
        JsonValue root;
    ...
    data_view_status_label_->SetLabel(
        wxString::Format("Data Views: %zu (stale: %zu)", total, stale));
}
...
bool DiagramPage::FocusNodeByName(const std::string& name) {
    return canvas_ ? canvas_->SelectNodeByName(name) : false;
}
...
bool DiagramPage::SaveToFile(const std::string& path, std::string* error) {
    if (doc_.diagram_id.empty()) {
        doc_.diagram_id = UUID::Generate().ToString();
    }
    SyncDocFromCanvas();
