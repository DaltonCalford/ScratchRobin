 ...
 */
#include "role_switch_policy_frame.h"
...
constexpr int kMenuConnect = wxID_HIGHEST + 2520;
constexpr int kMenuDisconnect = wxID_HIGHEST + 2521;
constexpr int kMenuRefresh = wxID_HIGHEST + 2522;
constexpr int kConnectionChoiceId = wxID_HIGHEST + 2523;
...
std::string FormatResult(const QueryResult& result) {
    if (result.rows.empty()) {
        return "No role switch policy rows returned.";
    }
...
wxBEGIN_EVENT_TABLE(RoleSwitchPolicyFrame, wxFrame)
    EVT_BUTTON(kMenuConnect, RoleSwitchPolicyFrame::OnConnect)
    EVT_BUTTON(kMenuDisconnect, RoleSwitchPolicyFrame::OnDisconnect)
    EVT_BUTTON(kMenuRefresh, RoleSwitchPolicyFrame::OnRefresh)
    EVT_CLOSE(RoleSwitchPolicyFrame::OnClose)
wxEND_EVENT_TABLE()

RoleSwitchPolicyFrame::RoleSwitchPolicyFrame(WindowManager* windowManager,
                                             ConnectionManager* connectionManager,
                                             const std::vector<ConnectionProfile>* connections,
                                             const AppConfig* appConfig)
    : wxFrame(nullptr, wxID_ANY, "Role Switch Policy",
...
RoleSwitchPolicyFrame::~RoleSwitchPolicyFrame() {
    if (window_manager_) {
        window_manager_->UnregisterWindow(this);
    }
}

void RoleSwitchPolicyFrame::BuildMenu() {
...
void RoleSwitchPolicyFrame::BuildLayout() {
...
void RoleSwitchPolicyFrame::PopulateConnections() {
...
void RoleSwitchPolicyFrame::UpdateStatus(const wxString& status) {
    if (status_label_) {
        status_label_->SetLabel("Status: " + status);
    }
}

void RoleSwitchPolicyFrame::SetMessage(const std::string& message) {
    if (message_label_) {
        message_label_->SetLabel(message);
    }
}

const ConnectionProfile* RoleSwitchPolicyFrame::GetSelectedProfile() const {
...
bool RoleSwitchPolicyFrame::EnsureConnected(const ConnectionProfile& profile) {
...
std::string RoleSwitchPolicyFrame::BuildQuery() const {
    return "SELECT key, value\n"
           "FROM sys.security_settings\n"
           "WHERE key IN ('role_switch_policy', 'role_switch_default_action')\n"
           "ORDER BY key;";
}

void RoleSwitchPolicyFrame::RefreshPolicy() {
    ...
    if (NormalizeBackendName(profile->backend) != "native") {
        SetMessage("Role switch policy queries are supported for ScratchBird connections.");
        return;
    }
...
void RoleSwitchPolicyFrame::OnConnect(wxCommandEvent&) {
    const auto* profile = GetSelectedProfile();
    if (!profile) {
        SetMessage("Select a connection profile first.");
        return;
...
void RoleSwitchPolicyFrame::OnDisconnect(wxCommandEvent&) {
    if (connection_manager_) {
...
void RoleSwitchPolicyFrame::OnRefresh(wxCommandEvent&) {
    RefreshPolicy();
}

void RoleSwitchPolicyFrame::OnClose(wxCloseEvent& event) {
    Destroy();
    event.Skip(false);
}
