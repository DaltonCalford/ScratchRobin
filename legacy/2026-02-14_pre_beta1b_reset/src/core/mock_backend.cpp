...
public:
    ...
    bool ExecuteCopy(const CopyOptions& options, std::istream*,
                     std::ostream*, CopyResult* outResult,
                     std::string* error) override {
        if (!connected_) {
            if (error) {
                *error = "Mock backend not connected";
        ...
        if (options.sql.empty()) {
            if (error) {
                *error = "COPY requires SQL";
            }
            return false;
        }
        if (outResult) {
            outResult->rowsProcessed = 0;
            outResult->commandTag = "COPY";
            outResult->message = "Mock COPY not executed";
        }
        return true;
    }
    ...
    bool Subscribe(const std::string& channel, const std::string&,
                   std::string* error) override {
        if (!connected_) {
            if (error) {
                *error = "Mock backend not connected";
    ...
    bool Unsubscribe(const std::string& channel, std::string* error) override {
        ...
        subscribed_channels_.erase(
            std::remove(subscribed_channels_.begin(), subscribed_channels_.end(), channel),
    ...
    bool FetchStatus(StatusRequestKind kind, StatusSnapshot* outSnapshot,
        ...
        switch (kind) {
            case StatusRequestKind::ServerInfo:
                outSnapshot->entries.push_back({"Server", "Mock ScratchBird"});
                outSnapshot->entries.push_back({"Version", "0.0.0-mock"});
                break;
            case StatusRequestKind::ConnectionInfo:
                outSnapshot->entries.push_back({"Status", "Connected"});
                outSnapshot->entries.push_back({"Latency", "0ms"});
                break;
            case StatusRequestKind::DatabaseInfo:
                outSnapshot->entries.push_back({"Database", "mock"});
                outSnapshot->entries.push_back({"Tables", "0"});
                break;
            case StatusRequestKind::Statistics:
                outSnapshot->entries.push_back({"Queries", "0"});
                outSnapshot->entries.push_back({"Uptime", "0s"});
                break;
        }
        return true;
    }
