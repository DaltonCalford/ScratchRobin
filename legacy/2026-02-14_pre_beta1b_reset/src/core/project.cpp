...
std::string FormatIsoTimestamp(std::time_t value) {
    std::tm tm{};
    #if defined(_WIN32)
        gmtime_s(&tm, &value);
    #else
...
std::shared_ptr<ProjectObject> Project::RegisterDiagramObject(const std::string& diagram_name,
                                                              const std::string& design_path,
                                                              const std::string& diagram_type) {
    if (diagram_name.empty() || design_path.empty()) {
        return nullptr;
    }

    std::string type_key = diagram_type.empty() ? "diagram" : diagram_type;
    auto existing = FindObjectByDesignPath(this, design_path);
    std::string obj_path = "diagram/" + type_key + "/" + diagram_name;
    if (existing) {
        if (existing->name != diagram_name) {
            existing->RecordChange("name", existing->name, diagram_name, "diagram");
            existing->name = diagram_name;
        }
        if (existing->path != obj_path) {
            objects_by_path.erase(existing->path);
            existing->path = obj_path;
            objects_by_path[existing->path] = existing;
        }
        if (existing->design_file_path != design_path) {
            existing->RecordChange("design_file_path",
                                   existing->design_file_path,
                                   design_path,
                                   "diagram");
            existing->design_file_path = design_path;
        }
        if (existing->kind != "diagram") {
            existing->RecordChange("kind", existing->kind, "diagram", "diagram");
            existing->kind = "diagram";
        }
        is_modified_ = true;
        NotifyObjectChanged(existing->id, "updated");
        return existing;
    }

    auto obj = std::make_shared<ProjectObject>("diagram", diagram_name);
    obj->path = obj_path;
    obj->design_file_path = design_path;
    objects_by_id[obj->id] = obj;
    objects_by_path[obj->path] = obj;
    is_modified_ = true;
    NotifyObjectChanged(obj->id, "created");
...
Project::GovernanceDecision Project::EvaluateGovernance(const GovernanceContext& context) const {
    GovernanceDecision decision;
    const auto& governance = config.governance;

    if (!context.environment_id.empty()) {
        auto env_it = std::find_if(governance.environments.begin(),
                                   governance.environments.end(),
                                   [&](const ProjectConfig::GovernanceEnvironment& env) {
                                       return env.id == context.environment_id;
                                   });
        if (env_it != governance.environments.end()) {
            if (!env_it->allowed_roles.empty()) {
                if (std::find(env_it->allowed_roles.begin(),
                              env_it->allowed_roles.end(),
                              context.role) == env_it->allowed_roles.end()) {
                    decision.allowed = false;
                    decision.reason = "Role not permitted in environment";
                    return decision;
                }
            }
            if (env_it->approval_required) {
                uint32_t required = env_it->min_reviewers;
                if (governance.review_policy.min_reviewers > required) {
                    required = governance.review_policy.min_reviewers;
                }
                if (context.approvals < required) {
                    decision.allowed = false;
                    decision.reason = "Insufficient approvals for environment";
                    return decision;
                }
            }
        }
    ...
    if (context.ai_action) {
        if (!governance.ai_policy.enabled) {
            decision.allowed = false;
            decision.reason = "AI assistance disabled";
            return decision;
        }
        if (!context.ai_scope.empty()) {
            if (!governance.ai_policy.allowed_scopes.empty() &&
                std::find(governance.ai_policy.allowed_scopes.begin(),
                          governance.ai_policy.allowed_scopes.end(),
                          context.ai_scope) == governance.ai_policy.allowed_scopes.end()) {
                decision.allowed = false;
                decision.reason = "AI scope not permitted";
                return decision;
            }
            if (std::find(governance.ai_policy.prohibited_scopes.begin(),
                          governance.ai_policy.prohibited_scopes.end(),
                          context.ai_scope) != governance.ai_policy.prohibited_scopes.end()) {
                decision.allowed = false;
                decision.reason = "AI scope prohibited";
                return decision;
            }
        }
    }
...
void Project::RecordReportingAudit(const AuditEvent& event) {
    if (project_root_path.empty()) {
        return;
    }
    std::filesystem::path log_path = std::filesystem::path(project_root_path) / "audit.log";
    std::ofstream out(log_path, std::ios::app);
    if (!out) {
        EmitStatus("Audit log write failed", true);
        return;
    }
    std::ostringstream line;
    line << "{";
    line << "\"timestamp\":" << static_cast<int64_t>(event.timestamp) << ",";
    line << "\"actor\":\"" << event.actor << "\",";
    line << "\"action\":\"" << event.action << "\",";
    line << "\"target_id\":\"" << event.target_id << "\",";
    line << "\"connection_ref\":\"" << event.connection_ref << "\",";
    line << "\"success\":" << (event.success ? "true" : "false") << ",";
...
bool Project::CanExecuteReportingAction(const GovernanceContext& context, std::string* reason) const {
...
bool Project::RefreshDataViewWithGovernance(const UUID& id,
                                            const std::string& captured_at_iso,
                                            const GovernanceContext& context,
                                            std::string* reason) {
    if (!CanExecuteReportingAction(context, reason)) {
        RecordReportingAudit(AuditEvent{
            std::time(nullptr),
            context.role,
            "refresh",
            id.ToString(),
    ...
    std::string timestamp = captured_at_iso.empty()
                                ? FormatIsoTimestamp(std::time(nullptr))
                                : captured_at_iso;
    if (!MarkDataViewRefreshed(id, timestamp)) {
        if (reason) *reason = "Data View not found";
        RecordReportingAudit(AuditEvent{
            std::time(nullptr),
            context.role,
            "refresh",
            ...
            "",
            false,
            reason ? *reason : "Data View not found"
        });
        return false;
    }
    for (auto& view : data_views) {
        if (view.id == id) {
            JsonParser parser(view.json_payload);
            JsonValue root;
            std::string error;
            if (parser.Parse(&root, &error)) {
                const JsonValue* query_value = FindMember(root, "query");
                if (query_value && query_value->type == JsonValue::Type::String &&
                    !query_value->string_value.empty()) {
                    auto refs = ExtractQueryReferences(query_value->string_value);
                    if (refs.parsed && !refs.identifiers.empty()) {
                        std::string updated;
                        if (UpdateJsonObjectStringArrayField(
                                view.json_payload, "query_refs", refs.identifiers, &updated)) {
                            view.json_payload = updated;
                            is_modified_ = true;
                        }
                    }
                }
...
bool Project::ScheduleReportingAction(const std::string& action,
...
Project::ReportingAsset* Project::UpsertReportingAsset(const ReportingAsset& asset) {
    for (auto& existing : reporting_assets) {
        if (existing.id == asset.id) {
            existing.object_type = asset.object_type;
            existing.json_payload = asset.json_payload;
            is_modified_ = true;
            return &existing;
        }
    }
    reporting_assets.push_back(asset);
    is_modified_ = true;
    return &reporting_assets.back();
}

Project::ReportingAsset* Project::InsertReportingAsset(const ReportingAsset& asset) {
    reporting_assets.push_back(asset);
    is_modified_ = true;
    return &reporting_assets.back();
}
...
int ParseScheduleIntervalSeconds(const std::string& spec) {
    std::string lower = ToLowerCopy(spec);
    if (lower == "hourly") return 3600;
    if (lower == "daily") return 86400;
    if (lower == "weekly") return 604800;
    if (lower.rfind("every ", 0) == 0) {
        std::istringstream iss(lower);
        std::string every;
        int value = 0;
        std::string unit;
        iss >> every >> value >> unit;
        if (value <= 0 || unit.empty()) {
            return 0;
        }
...
Project::ReportingSchedule* Project::AddReportingSchedule(const std::string& action,
                                                          const std::string& target_id,
    ...
    schedule.schedule_spec = schedule_spec;
    schedule.created_at = std::time(nullptr);
    schedule.interval_seconds = ParseScheduleIntervalSeconds(schedule_spec);
    if (schedule.interval_seconds > 0) {
        schedule.next_run = schedule.created_at + schedule.interval_seconds;
    }
    reporting_schedules.push_back(schedule);
    is_modified_ = true;
    EmitStatus("Reporting schedule created", false);
    return &reporting_schedules.back();
}

size_t Project::ExecuteDueReportingSchedules() {
    if (reporting_schedules.empty()) {
        return 0;
    }
    std::time_t now = std::time(nullptr);
    size_t executed = 0;
    for (auto& schedule : reporting_schedules) {
        if (!schedule.enabled) continue;
        if (schedule.interval_seconds <= 0) continue;
        if (schedule.next_run == 0 || schedule.next_run > now) continue;
        GovernanceContext context;
        context.action = "schedule_execute";
        context.role = "system";
        std::string reason;
        if (!CanExecuteReportingAction(context, &reason)) {
            RecordReportingAudit(AuditEvent{
    ...
    if (executed > 0) {
        EmitStatus("Reporting schedules executed: " + std::to_string(executed), false);
        is_modified_ = true;
    }
    return executed;
}

std::optional<Project::ReportingCacheEntry> Project::GetReportingCache(const std::string& key) const {
    auto it = reporting_cache.find(key);
    if (it == reporting_cache.end()) {
        return std::nullopt;
    }
    const auto& entry = it->second;
    if (entry.ttl_seconds > 0) {
        std::time_t now = std::time(nullptr);
        if (now > entry.cached_at && (now - entry.cached_at) > entry.ttl_seconds) {
            return std::nullopt;
        }
    }
    return entry;
...
void Project::StoreReportingCache(const ReportingCacheEntry& entry) {
    if (entry.key.empty()) {
        return;
    }
    ReportingCacheEntry stored = entry;
    if (stored.cached_at == 0) {
        stored.cached_at = std::time(nullptr);
    }
    reporting_cache[stored.key] = std::move(stored);
    if (reporting_cache.size() > 200) {
        auto oldest = reporting_cache.begin();
        for (auto it = reporting_cache.begin(); it != reporting_cache.end(); ++it) {
            if (it->second.cached_at < oldest->second.cached_at) {
                oldest = it;
            }
        }
        if (oldest != reporting_cache.end()) {
            reporting_cache.erase(oldest);
        }
    }
}

void Project::ClearReportingCache() {
    reporting_cache.clear();
}

Project::DataViewSnapshot* Project::UpsertDataView(const DataViewSnapshot& view) {
    for (auto& existing : data_views) {
        if (existing.id == view.id) {
            existing.diagram_id = view.diagram_id;
            existing.json_payload = view.json_payload;
            is_modified_ = true;
            return &existing;
        }
    }
    ...
    is_modified_ = true;
    return &data_views.back();
}

bool Project::MarkDataViewStale(const UUID& id, const std::string& reason) {
    for (auto& view : data_views) {
        if (view.id == id) {
            std::string updated;
            bool ok = UpdateJsonObjectBoolField(view.json_payload, "stale", true, &updated);
            if (ok && !reason.empty()) {
                UpdateJsonObjectStringField(updated, "stale_reason", reason, &updated);
            }
            if (ok) {
                view.json_payload = updated;
                is_modified_ = true;
...
bool Project::MarkDataViewRefreshed(const UUID& id, const std::string& captured_at_iso) {
    for (auto& view : data_views) {
        if (view.id == id) {
            std::string updated;
            bool ok = UpdateJsonObjectBoolField(view.json_payload, "stale", false, &updated);
            if (ok && !captured_at_iso.empty()) {
                UpdateJsonObjectStringField(updated, "captured_at", captured_at_iso, &updated);
                UpdateJsonObjectStringField(updated, "last_refresh", captured_at_iso, &updated);
            }
            if (ok) {
                view.json_payload = updated;
                is_modified_ = true;
...
void Project::InvalidateAllDataViews(const std::string& reason) {
    for (const auto& view : data_views) {
        MarkDataViewStale(view.id, reason);
    }
}

void Project::InvalidateDataViewsForObject(const std::string& schema, const std::string& name) {
    for (const auto& view : data_views) {
        ...
        bool matched = false;
        const JsonValue* refs_value = FindMember(root, "query_refs");
        if (refs_value && refs_value->type == JsonValue::Type::Array) {
            std::string schema_l = ToLowerCopy(schema);
            std::string name_l = ToLowerCopy(name);
            std::string full = schema_l.empty() ? name_l : (schema_l + "." + name_l);
            for (const auto& ref : refs_value->array_value) {
                if (ref.type != JsonValue::Type::String) continue;
                if (ref.string_value == name_l || ref.string_value == full) {
                    matched = true;
                    break;
                }
                if (!schema_l.empty()) {
                    size_t dot = ref.string_value.rfind('.');
                    if (dot != std::string::npos &&
                        ref.string_value.substr(dot + 1) == name_l) {
                        matched = true;
                        break;
                    }
                }
        ...
        if (matched) {
            MarkDataViewStale(view.id, "Schema change");
            continue;
        }
        const JsonValue* query_value = FindMember(root, "query");
        std::string query;
        if (query_value && query_value->type == JsonValue::Type::String) {
            query = query_value->string_value;
        }
        if (query.empty()) {
            continue;
        }
        if (QueryReferencesObject(query, schema, name)) {
            MarkDataViewStale(view.id, "Schema change");
        }
    }
}

std::vector<Project::TemplateAsset> Project::DiscoverTemplates(std::string* error) const {
    std::vector<TemplateAsset> results;
    if (project_root_path.empty()) {
        if (error) *error = "Project root not set";
        return results;
    }
    std::filesystem::path base = std::filesystem::path(project_root_path) /
                                 config.docs_path / "templates";
    if (!std::filesystem::exists(base)) {
        if (error) *error = "Templates directory not found";
        return results;
    }
    ...
    for (const auto& entry : std::filesystem::recursive_directory_iterator(base)) {
        ...
        asset.path = std::filesystem::relative(path, project_root_path).string();
        std::string lower = asset.name;
        std::transform(lower.begin(), lower.end(), lower.begin(),
                       [](unsigned char c) { return static_cast<char>(std::tolower(c)); });
        if (lower.find("mop") != std::string::npos) {
            asset.kind = "mop";
        } else if (lower.find("rollback") != std::string::npos) {
            asset.kind = "rollback";
...
bool Project::ModifyObject(const UUID& id, const MetadataNode& new_design) {
    ...
    if (!data_views.empty()) {
        std::string schema = obj->schema_name;
        std::string name = obj->name;
        if (!name.empty()) {
            InvalidateDataViewsForObject(schema, name);
        } else {
            InvalidateAllDataViews("Schema change");
        }
    }
...
Project::ValidationResult Project::Validate() const {
    ...
    if (project_root_path.empty()) {
        result.valid = false;
        result.errors.push_back("Project root path is not set.");
    }
    if (objects_by_id.empty()) {
        result.warnings.push_back("Project contains no objects.");
    }
    if (reporting_assets.empty()) {
        result.warnings.push_back("No reporting assets configured.");
    }
    return result;
}
...
bool Project::ApproveObject(const UUID& id, const std::string& approver) {
    ...
    if (!obj->design_state.CanTransitionTo(ObjectState::APPROVED) &&
        obj->design_state.CanTransitionTo(ObjectState::PENDING)) {
        obj->SetState(ObjectState::PENDING, "Pending approval", approver);
    }
...
bool Project::ApproveObjectWithGovernance(const UUID& id,
    ...
    return ApproveObject(id, approver);
}

bool Project::RejectObjectWithGovernance(const UUID& id,
                                         const std::string& reason,
                                         const GovernanceContext& context,
                                         std::string* out_reason) {
    GovernanceDecision decision = EvaluateGovernance(context);
    if (!decision.allowed) {
        if (out_reason) *out_reason = decision.reason;
        return false;
    }
    return RejectObject(id, reason);
}
