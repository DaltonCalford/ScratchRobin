...
## 6.2 Implementation Wiring Notes

These notes capture how enforcement should be wired in the ScratchRobin codebase:

- **Project Gate**: All governed actions must evaluate project roles + environment gates before mutation.
- **Reporting Gate**: Query execution, refresh, alert evaluation, and subscription delivery must call a common permission checker.
- **Deployment Gate**: Plan generation and execution must assert approvals from governance metadata.
- **Audit Hook**: Every governed action emits an audit entry with actor, scope, and decision (allow/deny).
- **Policy Changes**: Any governance policy update invalidates cached reporting results.
