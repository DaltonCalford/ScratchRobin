...
Wrapper types: JSONB uses `scratchbird::types::Jsonb`, RANGE uses `scratchbird::types::Range<T>`, GEOMETRY uses `scratchbird::types::Geometry`. Use these wrappers when binding parameters or reading results.
