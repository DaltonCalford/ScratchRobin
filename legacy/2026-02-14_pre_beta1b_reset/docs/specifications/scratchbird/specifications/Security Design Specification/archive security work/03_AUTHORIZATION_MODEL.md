...
│                                                                  │
│  Note: RLS is applied at SBLR generation time, not execution     │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```
...
The `sys.*` schemas contain system catalogs and are protected:
...
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  sys.catalog.*       Core metadata (tables, columns, indexes)    │
│  sys.sec.*           Security catalog (users, roles, privileges) │
│  sys.audit.*         Audit logs                                  │
│  sys.config.*        Configuration                               │
│                                                                  │
│  Access Rules:                                                   │
│                                                                  │
│  • Direct SELECT on sys.* tables: DENIED                        │
│    (even for superusers in strict mode)                          │
│                                                                  │
│  • Access via SHOW commands: FILTERED by permissions            │
│    User sees only objects they have privileges on                │
│                                                                  │
│  • Access via information_schema views: FILTERED                │
│    PostgreSQL-compatible views with permission filtering         │
│                                                                  │
│  • Superuser with CATALOG_ADMIN: Full access                    │
