...
| [OFFLINE_EXECUTION_CHECKLIST.md](OFFLINE_EXECUTION_CHECKLIST.md) | Short checklist of offline-only execution tasks with owners and status |
| [WIRE_PROTOCOL_EXECUTION_ROADMAP.md](WIRE_PROTOCOL_EXECUTION_ROADMAP.md) | Phase-by-phase roadmap for SBWP v1.1 enablement (COPY/CANCEL/PREPARED/STREAMING) |
| [WIRE_PROTOCOL_TRACKER.md](WIRE_PROTOCOL_TRACKER.md) | Task tracker for wire protocol enablement |

## Quick Reference

### Test/Perf Controls

- Performance tests are gated by `SCRATCHROBIN_RUN_PERF_TESTS=1`.
- Performance thresholds can be scaled with `SCRATCHROBIN_PERF_MS_FACTOR` (e.g., `2.0` doubles limits).
- CTest label: `perf` (and `quarantine` for perf-sensitive runs).
